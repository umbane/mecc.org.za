...
$first_name = $middle_name = $last_name = $email = $mobile = $sex = $age = $nat = $address = $suburb = $code = $city = $province = $country = $education = $skills = $answer1 = $agree = "";
$first_name_err = $middle_name_err = $last_name_err = $email_err = $mobile_err = $sex_err = $age_err = $nat_err = $address_err = $suburb_err = $code_err = $city_err = $province_err = $country_err = $education_err = $skills_err = $answer1_err = $agree_err = "";
...
if (empty(trim($_POST["nat"]))) {
               $age_err = "Please enter your residence status.";
            } else {
                $age = trim($_POST["nat"]);
         }
...
// Check input errors before inserting in the database
    if (empty($first_name_err) && empty($middle_name_err) && empty($last_name_err) && empty($email_err) && empty($mobile_err) && empty($sex_err) && empty($age_err) && empty($nat_err) && empty($address_err) && empty($suburb_err) && empty($code_err) && empty($city_err) && empty($province_err) && empty($country_err) && empty($education_err) && empty($skills_err) && empty($answer1_err) && empty($agree_err)) {

        $sql = "INSERT INTO members (first_name, middle_name, last_name, email, mobile, sex, age, nat, address, suburb, code, city, province, country, education, skills, answer1, agree) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssssssssssssss", $param_first_name, $param_middle_name, $param_last_name, $param_email, $param_mobile, $param_sex, $param_age, $param_nat, $param_address, $param_suburb, $param_code, $param_city, $param_province, $param_country, $param_education, $param_skills, $param_answer1, $param_agree);
...
<span class="quote">"Power to the People"</span>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
	     ...
	     <div class="form-group <?php echo (!empty($sex_err)) ? 'has-error' : ''; ?>">
            ...
            <div class="form-group <?php echo (!empty($age_err)) ? 'has-error' : ''; ?>">
             ...
             <div class="form-group <?php echo (!empty($nat_err)) ? 'has-error' : ''; ?>">
                    <label>Residence Status</label><br>
		   <label class="fsize12">National or Permanent Resident</label> <input type="radio" name="answer2" class="form-control" value="nat" <?php if ($answer2 === 'option1') echo 'checked'; ?>><br>

                   <label class="fsize12">Non-National or Non-Resident</label><input type="radio" name="answer2" class="form-control" value="non" <?php if ($answer2 === 'option2') echo 'checked'; ?>><br>
                
                  <span class="help-block"><?php echo $nat_err; ?></span>
              </div>
...
<!-- need to debug the js script to provide automated 
...
-->
	  ...
	  <div class="form-group <?php echo (!empty($answer1_err)) ? 'has-error' : ''; ?>">
                    ...
                    <label class="fsize12">Member</label><input type="radio" name="answer1" class="form-control" value="option1" <?php if ($answer1 === 'option1') echo 'checked'; ?>><br>

                     <label class="fsize12">Supporter</label><input type="radio" name="answer1" class="form-control" value="option2" <?php if ($answer1 === 'option2') echo 'checked'; ?>><br>

                     <label class="fsize12">Affiliate</label><input type="radio" name="answer1" class="form-control" value="option3" <?php if ($answer1 === 'option3') echo 'checked'; ?>><br>
